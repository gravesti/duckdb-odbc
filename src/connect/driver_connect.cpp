...
SQLRETURN SQL_API SQLDriverConnect(SQLHDBC connection_handle, SQLHWND window_handle, SQLCHAR *in_connection_string,
	...
	std::string in_conn_str = OdbcUtils::ConvertSQLCHARToString(in_connection_string);
	...
	if (nullptr != out_connection_string) {
		int written = std::snprintf(reinterpret_cast<char *>(out_connection_string),
		                            static_cast<std::size_t>(buffer_length), "%s", in_conn_str.c_str());
		if (nullptr != string_length2_ptr) {
			*string_length2_ptr = static_cast<SQLSMALLINT>(written);
		}
	} else if (nullptr != string_length2_ptr) {
